...
def parse_args():
    ...
    parser.add_argument('-e', '--edge_feat', type=bool, help="True to use channel as graph edge features, False otherwise", default=False)
    ...
    parser.add_argument('-v1', '--first_version', help="True to use combiner as antenna node features", action="store_true")

    return parser.parse_args()



def generate_pilots(Nrf, Nt, Nue, P, dl,f):
    # Base station params
...
if __name__ == "__main__":
    ...
    if args.first_version:
        ...
        # graph_data = {('antenna', 'channel2ue', 'user'): (src1, dst1),
        #               ('user', 'channel2an', 'antenna'): (src2, dst2),
        #               ('rfchain', 'psn2an', 'antenna'): (src3, dst3),
        #               ('antenna', 'psn2rf', 'rfchain'): (src4, dst4),
        #               ('user', 'pilot2rf', 'rfchain'): (src5, dst5),
        #               ('rfchain', 'pilot2ue', 'user'): (src6, dst6)}
    ...
    # when using a fied combiner set
    # indices = []
    # for i in range(Q):
    ...
    for n in range(args.num_graphs):
        ...
        W, Y, H = generate_pilots(Nrf, Nt, Nue, curr_comb, dl,f)
        psn_dev.append(W)
        pilots.append(Y)
        channel.append(H)
        # when not using a fied combiner set
    ...
    for i in range(args.num_graphs):
        ...
        # Channel edge features
            ...
            # g.edges['channel2an'].data['e'] = torch.tensor(feat_channel2an[i]) # use as feats for uplink edges (feature vecotr of size 2)
            # g.edges['channel2ue'].data['e'] = torch.tensor(feat_channel2ue[i]) # use as feats for downlink edges (feature vecotr of size 2)
        ...
        # PSN edge features (combiner)
            ...
            # # g.edges['psn2an'].data['e'] = torch.tensor(feat_psn2an) # (feature vecotr of size 2*Q)
            ...
            # # g.edges['psn2rf'].data['e'] = torch.tensor(feat_psn2rf) # (feature vecotr of size 2*Q)
        ...
        # pilots edge features
            # y = pilots[i].reshape(Q,Nrf,Nue) # shape: Q X Nrf X Nue 
            # feat_pilot2rf = np.hstack([y.transpose(0,2,1).reshape(Q,-1).T.real, y.transpose(0,2,1).reshape(Q,-1).T.imag], dtype=np.float32) # shape: Nue*Nrf X 2*Q
            # g.edges['pilot2rf'].data['e'] = torch.tensor(feat_pilot2rf)
            # feat_pilot2ue = np.hstack([y.reshape(Q,-1).T.real, y.reshape(Q,-1).T.imag], dtype=np.float32) # shape: Nue*Nrf X 2*Q
            # g.edges['pilot2ue'].data['e'] = torch.tensor(feat_pilot2ue)

            # g.edges['pilot2rf'].data['e'] = torch.tensor(feat_pilot2rf[i]) # (feature vecotr of size 2*Q)
            # g.edges['pilot2ue'].data['e'] = torch.tensor(feat_pilot2ue[i]) # (feature vecotr of size 2*Q)
