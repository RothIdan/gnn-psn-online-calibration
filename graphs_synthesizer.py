...
def generate_pilots(Nrf, Nt, Nue, N, phases_list, dl):
    ...
    Y, P = [], []
    for n in range(N):
        # Generating a random combiner for each measurement    
        idx = np.random.randint(0, len(phases_list), size=(Nrf,Nt))
        Pn = phases_list[idx]
        Pn = np.exp(1j * Pn) # Analog combiner is unit modulus
        P.append(Pn)

        # Generating recieved pilots according to system model
        Yn = np.matmul(np.multiply(W, Pn), H) # dtype = complex64, shape: Nrf X Nue
        Y.append(Yn)
    
    # Stacking matrices vertically
    Y = np.array(Y).reshape(N*Nrf, -1) # shape: M*Nrf X Nue
    P = np.array(P).reshape(N*Nrf, -1) # shape: M*Nrf X Nt
    ...
    # Y = np.vstack([Y.real, Y.imag], dtype=np.float32) # dtype = float32, shape: 2*Nrf X Nue
...
if __name__ == "__main__":
    ...
    N = Nt # no. of measurements
    dl = 1 # no. of channel multipath
    snr_db = 40 # For no AWGN use the value: None
    B = 5

    # Possible phases with a B-bit phase shifters
    delta = (2*np.pi) / (2**B)
    ...
    for n in range(args.num_graphs):
        ...
        W, Y, H, P = generate_pilots(Nrf, Nt, Nue, N, phases_list, dl)
        psn_dev.append(W)
        pilots.append(Y)
        channel.append(H)
        combiner.append(P)
    ...
    ue_feat = np.transpose(np.array(pilots), (0,2,1))
    ue_feat = np.concatenate((ue_feat.real, ue_feat.imag), axis=2)  
    
    if args.antenna_feat:
        antenna_feat = np.transpose(np.array(combiner), (0,2,1))
        antenna_feat = np.concatenate((antenna_feat.real, antenna_feat.imag), axis=2)
    
    # Data normalization
    if args.norm:
        mean, std = ue_feat.mean(axis=0), ue_feat.std(axis=0)
        ue_feat = (ue_feat-mean) / std
        if args.antenna_feat:
    ...
    for i in range(args.num_graphs):
        ...
        g.nodes['user'].data['feat'] = torch.tensor(ue_feat[i]) # shape: Nue X 2*N*Nrf
        if args.antenna_feat:
            g.nodes['antenna'].data['feat'] = torch.tensor(antenna_feat[i]) # shape: Nt X 2*N*Nrf
        graphs.append(g)
    ...
    #         W, Y, H, P = generate_pilots(Nrf, Nt, Nue, M, dl, snr_db)
    #         psn_dev.append(W)
    #         pilots.append(Y)
    #         channel.append(H)
    #         combiner.append(P)
